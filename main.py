...
if __name__ == "__main__":
    ...
    parser.add_argument('--initial_filename', type=str, required=True,
                        help='The initial filename (e.g., makefile_test.am, initial_big_data.json')
    parser.add_argument('--work_folder', type=str, required=True,
                        help='The working folder where temporary files are stored')
    parser.add_argument('--s1_key', type=str, required=True,
                        help='The key representing S1 (e.g., S1, S1.cpp) for blackbox')
    parser.add_argument('--s2_key', type=str, required=True,
                        help='The key representing S2 (e.g., S2, S2.cpp) for blackbox')
    ...
    parser.add_argument('--cleanup', action='store_true', required=False,
                        help='If set, will clean up the work folder after processing')
    ...
    if args.mode == "json":
        ...
        makefile_mode(args.initial_filename, args.work_folder, args.s1_key, args.s2_key, args.cleanup,
                      args.blackbox_path)
    else:
